...
def main(program, nsteps, nmax, temp, pflag):
    ...
    if taskid==0:
        ...
        for it in range(1,nsteps+1):
            ...
            comm.send(v, dest=2, tag=it)
                        
            
    if taskid==1:
        for w in range(0,nsteps+1):
            data = comm.recv(source=0, tag=w)
            empi = all_energy(data,nmax)
            energy_mpi[w]=empi
        v=np.array(energy_mpi,dtype=float)
        comm.send(v,dest=0, tag=nsteps+1) 
    
    if taskid==2:
        for o in range(0,nsteps+1):
            data = comm.recv(source=0, tag=o)
            ompi = get_order(data,nmax)
            order_mpi[o]=ompi
        o = np.array(order_mpi,dtype=float)
        comm.send(o, dest=0, tag=nsteps+3)

        
        
    if taskid==0:
        energy = comm.recv(source=1, tag=nsteps+1)
        order = comm.recv(source=2, tag=nsteps+3)
