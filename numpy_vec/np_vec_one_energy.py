...
# to the energy
    ...
    #array of the neighbouring angles
    angs = np.array([ang1, ang2, ang3, ang4])
    
    #array of neighbours energies
...
def MC_step(arr,Ts,nmax, seed=None):
    ...
    accept = 0
    rng = np.random.default_rng(seed=seed)
    xran = rng.integers(low=0,high=nmax,size=(nmax,nmax))
    
    rng = np.random.default_rng(seed=seed)
    yran = rng.integers(low=0,high=nmax,size=(nmax,nmax))
